...
class FlowDashboard:
    ...
    - Zone du milieu : tableau (heure, caméra, mesure 3s) avec scroll
    ...
    def show_summary(
        ...
        self._setup_plot()
        self._draw(history, prediction_total, prediction_a, prediction_b, 
                   current_a, current_b, trend, trend_a, trend_b, session_records)

        # Créer fenêtre Tkinter avec scrollbar
    ...
    def _create_scrollable_window(self):
        ...
        # Intégrer le graphique Matplotlib dans le canvas
    ...
    def _draw(
        ...
        try:
            ...
            # ---------------------------
            # 2) Zone du bas : tableau COMPLET (3s) sans limitation
            # ---------------------------
            self._draw_table(session_records)

            # ---------------------------
            # 3) Panneau d'information (stats globales + flux + prédictions par caméra)
            # ---------------------------
    ...
    def _draw_table(self, session_records: List[Dict[str, Any]]):
        """
        Dessine un tableau COMPLET dans ax2 (toutes les lignes, pas de limitation).
        Le scroll permettra de naviguer.
        """
        self.ax2.clear()
        self.ax2.axis('off')
        ...
        try:
            ...
            # PLUS DE LIMITATION : afficher toutes les lignes
    ...
    def _estimate_flux_ab(
        ...
        for i in range(n - delay_steps - 1):
            ...
            a_variation_significant = (
                ...
                (a_now > 0.1 and delta_a_immediate / a_now >= 0.3)
            )
            
            b_increase_significant = (
                delta_b_delayed >= threshold or
                (b_now >= 0 and delta_b_delayed > 0.1)
            )
...
class FrameAnnotator:
    ...
    def annotate_frame(
        ...
        cv2.putText(annotated, text1, (15, 35), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
        cv2.putText(annotated, text2, (15, 75), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)

        timestamp = datetime.now().strftime("%H:%M:%S")
        cv2.putText(annotated, timestamp, (w - 150, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)
